...
namespace ITDeskServer.Controllers
{
    ...
    {
        //TicketController'ın içine yazdığımız aşağıdaki 4 satırlık kod ile aynı işlevi yapmaktadır.
        ...
        public IActionResult Add([FromForm]TicketAddDto request)
        {
            ...
            if(request.Files is not null)
            {
                ...
                foreach(var file in request.Files)
                {
                    ...
                    using (var stream = System.IO.File.Create(@"C:\YMYP-Angular\ITDesk\ITDeskClient\src\assets\files\"+ fileName))
                    {
                        file.CopyTo(stream);
                    }
